...
- `twl_llp = \(n) 1.75 * log(n) / n` applies the calibrated pooling weight for workflows combining long and daily stationary periods. Do not transfer this value automatically to workflows with multiple stationary-period durations defined from acceleration data.
...
An example of the resulting report is available on the [GeoPathSampleR pkgdown site](https://geopressure.org/GeoPathSampleR/sampling-path-diagnostic.html).
...
The same information is available as compact objects in the console:
```{r}
sampling_path_diagnostic(paths, tag, report = FALSE)
```

R-hat values close to 1, adequate effective sample sizes, overlapping chains, and no support warnings indicate that the run is behaving as expected. The short first run will normally be insufficient, so extend it after checking that the workflow is valid. The longer run is shown for interactive use and is excluded from the automatic vignette build:
